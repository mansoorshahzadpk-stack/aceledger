...
function AuditLog() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<{ id: string; type: string; ref: string } | null>(null);
        ...
        <CardContent>
          <div className="overflow-auto rounded-md border">
            <Table>
              ...
              <TableBody>
                {(data ?? []).length === 0 && <TableRow><TableCell colSpan={8} className="text-center text-muted-foreground py-8">No amendments yet</TableCell></TableRow>}
                {data?.map((r) => (
                  <TableRow key={r.id}>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => { if (!open) { setDeleteTarget(null); setMasterPassword(""); } }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              You are about to delete an audit log entry for <strong>{deleteTarget?.ref}</strong> ({deleteTarget?.type}). This operation requires verification of the Master Password.
            </DialogDescription>
          </DialogHeader>
