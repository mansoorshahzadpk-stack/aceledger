...
function ClientDetail() {
  ...
  const [paymentSortField, setPaymentSortField] = useState<
    "payment_date" | "status" | "amount" | null
  >("payment_date");
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      if (!activeBusinessId || !user)
        return { c: null, invs: [], pays: [], amends: [], outstanding: 0, posted: 0 };
      ...
      const paid = (pays ?? [])
        .filter((p) => (p.status || "posted") === "posted")
  ...
  const renderSortIndicator = (
    ...
    if (currentOrder === "desc")
      return <ChevronDown className="ml-1 h-3.5 w-3.5 inline shrink-0" />;
    return null;
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {data.c.phone ?? ""} {data.c.email ? ` · ${data.c.email}` : ""}
            </p>
          </div>
          <div className="flex items-center gap-3 rounded-md border bg-muted/30 px-4 py-2">
            ...
            <span className="figure text-xl font-semibold text-warning">
              {formatMoney(data.outstanding, settings.currency)}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap gap-2">
          <Button asChild className={isReadOnly ? "pointer-events-none opacity-50" : ""}>
            <Link to="/invoices/new" search={{ client: id } as any}>
          ...
          <Button
            variant="default"
            disabled={
              isReadOnly || (postedInvoices.length === 0 && Number(data.c.opening_balance) === 0)
            }
                ...
                <TableBody>
                  ...
                  {sortedInvoices.map((i) => (
                    <TableRow key={i.id} className="hover:bg-muted/30">
                      ...
                      <TableCell>
                        <Badge
                          variant={i.status === "posted" ? "default" : "secondary"}
                      ...
                      <TableCell className="text-right figure">
                        {formatMoney(i.total, settings.currency)}
                      </TableCell>
                      <TableCell className="text-right">
                        <Button asChild size="sm" variant="ghost">
                          <Link to="/invoices/$id" params={{ id: i.id }}>
                            <Pencil className="h-3.5 w-3.5 mr-1" />
                            {i.status === "posted" ? "Amend" : "Edit"}
                          </Link>
                        </Button>
                      </TableCell>
                    </TableRow>
        ...
        >
          <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
            <div className="space-y-1">
              ...
              <CardDescription>
                Payments received from this client — drafts can be updated/deleted freely
              </CardDescription>
            </div>
            <Button
              variant="ghost"
                ...
                <TableHeader>
                  <TableRow>
                    ...
                    >
                      <div className="flex items-center gap-1">
                        Date{" "}
                        {renderSortIndicator("payment_date", paymentSortField, paymentSortOrder)}
                      </div>
                    </TableHead>
      ...
      {data.amends.length > 0 && showHistory && (
        <Card className="min-w-0 animate-in fade-in slide-in-from-top-4 duration-300">
          <CardHeader>
            ...
            <CardDescription>Audit trail of edits and deletions</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="overflow-auto rounded-md border">
              <Table>
        ...
        <DialogContent className="max-w-4xl lg:max-w-5xl w-[90vw]">
          ...
          <form onSubmit={(e) => e.preventDefault()} className="space-y-3">
            ...
            <Field label="Apply to invoice (optional)">
              <Select
                value={pay.invoice_id || "none"}
                onValueChange={(v) => setPay({ ...pay, invoice_id: v === "none" ? "" : v })}
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  ...
                  {postedInvoices.map((i) => (
                    <SelectItem key={i.id} value={i.id}>
                      {i.invoice_number} · {formatMoney(i.total, settings.currency)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            {(editPay?.status || "posted") === "posted" && (
              <Field label="Reason">
                <Textarea
                  ...
                  placeholder="e.g. Corrected from bank receipt"
                />
              </Field>
            )}
          </div>
