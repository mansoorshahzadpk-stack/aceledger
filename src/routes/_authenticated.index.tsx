  ...
  head: () => ({
    meta: [
      { title: "Dashboard — Ace Ledger" },
      {
        name: "description",
        content:
          "Executive dashboard for outstanding receivables, vendor payables, and weekly collections in your B2B ledger.",
      },
    ...
    queryFn: async () => {
      if (!activeBusinessId || !user)
        return {
          ...
          weekCollections: 0,
          recent: [],
          counts: { clients: 0, vendors: 0, invoices: 0 },
        };
      const [clients, invoices, cpay, vendors, grns, vpayResult] = await Promise.all([
        ...
        supabase
          ...
          .select("id, amount, payment_date, method, client_id, clients(name), status")
          .eq("business_id", activeBusinessId)
          .eq("user_id", user.id)
      ...
      const postedTotal = (invoices.data ?? [])
        .filter((i) => i.status === "posted")
        .reduce((s, x) => s + Number(x.total), 0);
      const paidIn = (cpay.data ?? [])
      ...
      const weekCollections = (cpay.data ?? [])
        .filter((p) => p.status === "posted" && new Date(p.payment_date) >= weekAgo)
        .reduce((s, x) => s + Number(x.amount), 0);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Real-time inflow, outflow, and weekly collections
        </p>
      </div>
                ...
                {!isLoading && data?.recent.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={4} className="text-center text-muted-foreground py-8">
                      No payments yet. Head to{" "}
                      <Link to="/clients" className="underline">
                        Clients
                      </Link>{" "}
                      and log a weekly installment.
                    </TableCell>
                  </TableRow>
                )}
                  ...
                  <TableRow key={p.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1.5">
                        ...
                        <Badge
                          variant={(p.status || "posted") === "posted" ? "default" : "outline"}
                    ...
                    <TableCell className="text-right figure font-medium text-success">
                      {formatMoney(p.amount, c)}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
    ...
    <Card>
      <CardContent className="p-5">
        <div className="flex items-start justify-between">
          ...
          <div
            className={`flex h-10 w-10 items-center justify-center rounded-lg ${colorMap[tone]}`}
          >
