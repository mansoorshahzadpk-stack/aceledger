...
function VendorDetail() {
  ...
  const { data } = useQuery({
    queryKey: ["vendor", id],
    queryFn: async () => {
      const [{ data: v }, { data: grns }, { data: pays }] = await Promise.all([
      ...
      return { v, grns: grns ?? [], pays: pays ?? [], owed };
    },
    enabled: !!user,
  });
        ...
        <Card>
          ...
          <CardContent>
            <div className="overflow-auto rounded-md border">
              <Table>
                <TableHeader><TableRow><TableHead>Date</TableHead><TableHead>GRN #</TableHead><TableHead>Material</TableHead><TableHead className="text-right">Amount</TableHead><TableHead></TableHead></TableRow></TableHeader>
                <TableBody>
                  ...
                  {data.grns.map((g) => (
                    <TableRow key={g.id}>
                      ...
                      <TableCell><Button size="icon" variant="ghost" onClick={() => printGrn(g)}><Printer className="h-4 w-4" /></Button></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
